<template>
  <div class="organization-todos-view">
    <div class="view-header">
      <div class="header-left">
        <router-link :to="`/organizations/${route.params.id}`" class="back-link">
        ...
        <div class="header-title">
          ...
          <h2 v-else>Organization Tasks</h2>
          <div class="header-meta">
            <span class="view-description">
              All tasks from projects in this organization
            </span>
          </div>
        </div>
      </div>
    ...
    <div v-if="loading" class="loading-state">
      <p>Loading organization tasks...</p>
    </div>

    <div v-else-if="error" class="error-state">
      <p>{{ error }}</p>
      <button @click="loadOrganizationTodos" class="btn-primary">Retry</button>
    </div>

    <div v-else class="todos-content-container">
      <div v-if="todos.length === 0" class="empty-state">
        <p>No tasks linked to projects in this organization</p>
        <p class="empty-state-hint">
          Create a task and link it to a project in this organization to see it here
        </p>
      </div>
...
async function loadOrganizationTodos() {
  ...
  try {
    ...
    // Load todos for all projects in the organization
    const todosResponse = await organizationsApi.getTodos(organizationId, { skip: 0, limit })
...
async function loadMore() {
  const organizationId = route.params.id
  try {
    const response = await organizationsApi.getTodos(organizationId, { skip: currentSkip.value, limit })
...
:global(.dark) .organization-todos-view {
  background: #0f172a;
}
