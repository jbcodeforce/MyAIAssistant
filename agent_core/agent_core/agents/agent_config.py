...
LOCAL_MODEL = os.getenv("LOCAL_LLM_MODEL", "mistralai/ministral-3-14b-reasoning")
...
def get_available_models() -> list[str]:
    """Get list of available models in local server."""
    try:
        response = httpx.get(f"{LOCAL_BASE_URL}/../api/v1/models", timeout=5.0)
        if response.status_code == 200:
            models = response.json().get("models", [])
            return [m.get("key", "").split(":")[0]  for m in models]
        return []
