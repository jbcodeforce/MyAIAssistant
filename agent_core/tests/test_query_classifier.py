    ...
    async def test_classify_knowledge_search(self, classifier, mock_llm_response_knowledge):
        ...
        with patch.object(classifier, '_call_llm', new_callable=AsyncMock) as mock_call:
            ...
            result = await classifier.classify(
                "What does our documentation say about OAuth authentication?"
            )
...
class TestQueryClassifierLLMClient:
    ...
    def test_openai_config_has_json_format(self):
        """Test OpenAI config includes JSON response format."""
        classifier = QueryClassifier(
            provider="openai",
            model="gpt-4",
            api_key="test-key"
        ...
        assert classifier._llm_config.response_format == {"type": "json_object"}
    
    def test_ollama_config_no_json_format(self):
        """Test Ollama config doesn't use OpenAI-style JSON format."""
        classifier = QueryClassifier(
            provider="ollama",
            model="llama2"
        )
        
        # Ollama doesn't use OpenAI's response_format
