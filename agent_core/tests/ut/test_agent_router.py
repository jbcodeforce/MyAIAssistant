...
class TestAgentRouter:
    ...
    def router(self, mock_classifier, mock_rag_agent, mock_task_agent, 
        ...
        return AgentRouter(
            ...
            intent_mapping={
                ...
                QueryIntent.TASK_PLANNING: "task",
                QueryIntent.TASK_STATUS: "task",
    ...
    async def test_route_no_classifier_configured(self):
        ...
        # Explicitly set classifier to None to test the "no classifier" path
        # (AgentRouter.__init__ falls back to get_query_classifier() if None)
...
class TestAgentRouterIntentMapping:
    ...
    def test_default_intent_mapping(self):
        ...
        # Default mappings use intent names as agent type names
        assert router.intent_mapping[QueryIntent.KNOWLEDGE_SEARCH] == "knowledge_search"
        assert router.intent_mapping[QueryIntent.TASK_PLANNING] == "task_planning"
        assert router.intent_mapping[QueryIntent.TASK_STATUS] == "task_status"
        assert router.intent_mapping[QueryIntent.CODE_HELP] == "code_help"
        assert router.intent_mapping[QueryIntent.GENERAL_CHAT] == "general_chat"
        assert router.intent_mapping[QueryIntent.UNCLEAR] == "unclear"
